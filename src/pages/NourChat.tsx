...
function startingPrice(id: string) {
  ...
  const values = [item?.price, item?.priceManual, item?.pricePower].filter((value): value is number => typeof value === "number");
...
export default function NourChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: "welcome", role: "assistant", content: "I help you avoid the expensive mistake of choosing furniture that looks right but feels wrong in your home. Send one room photo and I’ll help you picture a real Dandle in it." },
  ]);
  ...
  const attachImage = async (file?: File) => {
    ...
    setRender(null);
    setRenderError("");
    try { setRoomImage(await compressImage(file)); } catch { setRenderError(isAr ? "الصورة ما اتفتحتش. جرب صورة تانية." : "That image could not be opened. Try another photo."); }
  };

  const send = async () => {
    const text = input.trim();
    if ((!text && !roomImage) || chatLoading) return;
    const userText = text || (isAr ? "اقرأ صورة المكان واقترح 3 أماكن شكلهم طبيعي للكرسي." : "Read this room photo and suggest 3 visually natural placements for the chair.");
    ...
    try {
      ...
      setMessages((current) => [...current, {
        ...
        content: isAr ? "خلينا نخليها بسيطة: اختار مكان واضح من الصورة، مثل الركن الفاضي جنب الكنبة، وأنا أستخدمه في التصور." : "Keep it simple: choose a visible open area, such as the empty corner beside the sofa, and I’ll use that for the visualization.",
      }]);
  ...
  const createVisualization = async () => {
    ...
    try {
      const response = await fetch("/api/nour-render", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ roomImage, modelId, modelName: selectedModel.name, material, colour, placement }),
      });
      const data = await response.json();
      if (!response.ok || !data.approved || !data.image) throw new Error(data.error || "Visualization failed quality check");
          ...
          <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-charcoal/70">
                {isAr ? "ارفع صورة واحدة للمكان. نور تعمل تصور واقعي للموديل داخل نفس الغرفة. ده تصور للشكل والإحساس، مش قياس هندسي أو AR." : "Upload one room photo. Nour creates a realistic visualization of the selected Dandle inside that same room. It is an appearance tool, not a measurement or AR fit check."}
              </p>
            </div>
          ...
          <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
            <section className="overflow-hidden rounded-2xl border border-bronze/20 bg-warm-white shadow-luxury">
              <div className="border-b p-4 md:p-6">
                <div className="flex items-center gap-3"><Sparkles className="h-5 w-5 text-accent" /><h2 className="text-2xl">{isAr ? "استوديو نور" : "Nour Room Studio"}</h2></div>
              </div>
              ...
              <div className="p-4 md:p-6">
                {!roomImage ? (
                  <button onClick={() => fileRef.current?.click()} className="flex min-h-80 w-full flex-col items-center justify-center rounded-2xl border border-dashed bg-background p-8 text-center transition hover:border-accent">
                    ...
                    <span className="mt-2 max-w-md text-sm text-muted-foreground">{isAr ? "صورة واضحة من الزاوية اللي تحب تشوف فيها الكرسي." : "Use a clear photo from the viewpoint where you want to see the recliner."}</span>
                  </button>
                ) : (
                  <div className="relative overflow-hidden rounded-2xl bg-black/5">
                    ...
                    {render && <div className="absolute bottom-3 left-3 flex items-center gap-2 rounded-full bg-white/95 px-3 py-2 text-xs font-semibold shadow"><CheckCircle2 className="h-4 w-4 text-emerald-600" />{isAr ? "اجتاز فحص نور" : "Passed Nour quality check"}</div>}
                  </div>
                )}
                ...
                {roomImage && !render && <Button variant="outline" className="mt-4" onClick={() => fileRef.current?.click()}><Paperclip className="mr-2 h-4 w-4" />{isAr ? "غير الصورة" : "Change room photo"}</Button>}
              </div>
            </section>
            ...
            <aside className="space-y-5">
              <section className="rounded-2xl border bg-warm-white p-5 shadow-subtle">
                ...
                {!selectedModel.ready && <p className="mt-3 rounded-lg bg-amber-50 p-3 text-xs text-amber-900">{isAr ? "مش هستخدم صورة غير موثقة أو AI كمرجع للمنتج. ارفع صورة المنتج الحقيقية عشان نفعل التصور لهذا الموديل." : "Nour will not use an unverified or AI product image as product truth. A clean real reference photo is required to activate visualization for this model."}</p>}
              </section>

              <section className="rounded-2xl border bg-warm-white p-5 shadow-subtle">
                <h2 className="text-2xl">{isAr ? "2. الخامة واللون" : "2. Material and colour"}</h2>
                <div className="mt-4 flex flex-wrap gap-2">
                  {MATERIALS.map((item) => <button key={item} onClick={() => { setMaterial(item); setRender(null); }} className={`rounded-full border px-3 py-2 text-xs ${material === item ? "border-accent bg-accent/10 text-accent" : "bg-background"}`}>{item}</button>)}
                </div>
                <p className="mt-3 text-xs leading-relaxed text-muted-foreground">{isAr ? "الخامات هنا أسماء موثقة فقط. صور swatch الحقيقية لسه مطلوبة لتحسين دقة الخامة، ومش هنخترع texture رقمي." : "These are verified material names only. Real swatch photography is still required for exact material fidelity; Nour will not invent digital swatches."}</p>
                {modelId === "diva" ? (
                  ...
                  <input value={colour} onChange={(event) => { setColour(event.target.value); setRender(null); }} placeholder={isAr ? "اللون المطلوب، لو عندك اختيار" : "Desired colour, if already chosen"} className="mt-4 w-full rounded-lg border bg-white px-3 py-2 text-sm" />
                )}
              </section>
              ...
              <section className="rounded-2xl border bg-warm-white p-5 shadow-subtle">
                ...
                <Button variant="outline" className="mt-3 w-full" onClick={() => send()} disabled={!roomImage || chatLoading}>{chatLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{isAr ? "اقترح أماكن من الصورة" : "Suggest placements from my photo"}</Button>
                <div className="mt-4 max-h-48 space-y-2 overflow-y-auto rounded-xl bg-background p-3 text-xs">
                  {messages.slice(-3).map((message) => <p key={message.id} className={message.role === "assistant" ? "text-foreground" : "text-muted-foreground"}>{message.content}</p>)}
                </div>
                <div className="mt-3 flex gap-2"><input value={input} onChange={(event) => setInput(event.target.value)} placeholder={isAr ? "اسأل نور..." : "Ask Nour..."} className="min-w-0 flex-1 rounded-lg border bg-white px-3 py-2 text-sm" /><Button size="icon" onClick={() => send()} disabled={chatLoading || (!input.trim() && !roomImage)}><Send className="h-4 w-4" /></Button></div>
              </section>

              <Button className="w-full py-6 text-base" onClick={createVisualization} disabled={!canRender}>
                {renderLoading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />{isAr ? "نور بتعمل وتراجع التصور..." : "Nour is creating and checking your visual..."}</> : <><Sparkles className="mr-2 h-5 w-5" />{isAr ? "شوفه في مكانك" : "Visualize it in my room"}</>}
              </Button>

              {render && <a href={whatsAppUrl} target="_blank" rel="noreferrer" className="flex items-center justify-center rounded-xl border border-accent bg-white px-4 py-4 font-semibold text-accent transition hover:bg-accent/5"><MessageCircle className="mr-2 h-5 w-5" />{isAr ? "كمل مع داندل على WhatsApp" : "Continue with Dandle on WhatsApp"}</a>}
            </aside>
          </div>
        </div>
      </main>
