...
const OpenAiAttributionHandler = () => {
  const location = useLocation();

  useEffect(() => {
    try {
      const oppref = new URLSearchParams(location.search).get("oppref")?.trim().slice(0, 500);
      if (oppref) window.sessionStorage.setItem("dandle:openai:oppref", oppref);
    } catch {
      // Attribution capture must never disrupt browsing or checkout.
    }
  }, [location.search]);
